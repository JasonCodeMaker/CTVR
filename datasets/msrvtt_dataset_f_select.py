...
class MSRVTTDataset(Dataset):
    ...
    """
    def __init__(self, config: Config, data, model, split_type = 'train', img_transforms=None, replayed_pairs=None):
        self.config = config
    ...
    def __getitem__(self, index):
        ...
        if self.split_type == 'train':
            video_id, text, frames = self._get_vidpath_and_caption_by_index(index)
    ...
    def _frame_reduction(self, top_k=6):
        ...
        for vid, caption in self.all_train_pairs:
            ...
            with torch.no_grad():
                ...
                frame_batch = frames.unsqueeze(1).to(self.device)
                frame_features = self.clip.forward_video(frame_batch)

                text_token = clip_tokenizer(caption, return_tensors='pt', padding=True,
                                                            truncation=True).to(self.device)
                text_features = self.clip.forward_text(text_token.input_ids, text_token.attention_mask)

            # Calculate cosine similarity
            similarities = torch.nn.functional.cosine_similarity(frame_features, text_features, dim=-1)
            
            # Select top k frame indices
            top_k_indices = similarities.argsort(descending=True)[:top_k].to('cpu').numpy()
            
            # Select top k frames
            top_k_frames = frames[top_k_indices]
            
            # Add reduced pair to new list
